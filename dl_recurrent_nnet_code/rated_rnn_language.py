...
class SimpleRNN:
    ...
    def set(self, We, Wx, Wh, bh, h0, Wxz, Whz, bz, Wo, bo, activation):
        ...
        def recurrence(x_t, h_t1): # do calculation at each recurrent unit
            ...
            z_t = T.nnet.sigmoid(x_t.dot(self.Wxz) + h_t1.dot(self.Whz) + self.bz) # using M hidden nodes to generate (1*M) probabilities as a filter
            h_t = (1 - z_t) * h_t1 + z_t * hhat_t     # filtering (to retain longer memory)
        ...
        py_x = y[:, 0, :]  # (T*V) matrix of estimated probabilities
    ...
    def generate(self, word2idx):
        ...
        while n_lines < 4:
            ...
            # choose a random word to start a new sentence. 
            # The choice probability comes from nnet softmax prediction.
            # Rather than using the prediction, we introduce randomness at each word to increase language variety.
