...
class LSTM:
    ...
    def recurrence(self, x_t, h_t1, c_t1): # do calculate at each recurrent unit
        ...
        c_t = f_t * c_t1 + i_t * T.tanh(x_t.dot(self.Wxc) + h_t1.dot(self.Whc) + self.bc)            # new cell value
        o_t = T.nnet.sigmoid(x_t.dot(self.Wxo) + h_t1.dot(self.Who) + c_t.dot(self.Wco) + self.bo)   # output gate
        h_t = o_t * T.tanh(c_t)  # new hidden state
        return h_t, c_t

    def output(self, x):
        # input X should be a matrix (T*D)
        # rows index time
        [h, c], _ = theano.scan( # do calculate at each recurrent unit
            fn=self.recurrence,
            sequences=x, # scan each row of X
